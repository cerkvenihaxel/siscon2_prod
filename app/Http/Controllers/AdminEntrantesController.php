...
//			$this->col[] = ["label"=> "Usuario Carga", "name"=>"userId"];
			...
			function proveedorAdminPrivilegeId(){
				...
				$privilege = CRUDBooster::myPrivilegeId();
				if($privilege != 1 && $privilege !=2 && $privilege != 3 && $privilege !=4 && $privilege != 5 && $privilege != 6 && $privilege != 17 && $privilege != 33 && $privilege != 34 && $privilege != 35 && $privilege != 37 && $privilege != 46){
					return true;
				}else{
					return false;
				}
...
//<<<<<<< HEAD
			...
			$this->form[] = ['label'=>'Número de Solicitud','name'=>'nrosolicitud','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10','required'=>true,'readonly'=>'true','value'=>'SISCON'.date('dmHis').'-'.rand(0, 99999)];
