<?php namespace App\Http\Controllers;
	...
	class AdminPedidosO21Controller extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "pedidos_o2";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
			$this->col = [];
			$this->col[] = ["label"=>"Afiliados Id","name"=>"afiliados_id","join"=>"afiliados,apeynombres"];
            $this->col[] = ["label"=>"Médico de carga", "name"=>"medicos_id", "join"=>"medicos,nombremedico"];
            $this->col[] = ["label"=>"Fecha de creación", "name"=>"created_at"];
            $this->col[] = ["label"=>"Número de solicitud", "name"=>"nro_solicitud"];
            ...
            $nroSolicitud = 'SISCON-O2-T-' . date('dmHis');

            # START FORM DO NOT REMOVE THIS LINE
			$this->form = [];
			$this->form[] = ['label'=>'Afiliados Id','name'=>'afiliados_id','type'=>'text','validation'=>'required|integer|min:0','width'=>'col-sm-10', 'value'=>$afiliadoId, 'readonly'=>true];
			$this->form[] = ['name'=>'cms_users_id', 'type'=>'hidden', 'value'=>$userID];
            $this->form[] = ['label'=>'Direccion','name'=>'direccion','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Medicos solicitante','name'=>'medicos_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'medicos,nombremedico'];
			$this->form[] = ['label'=>'Nro Solicitud','name'=>'nro_solicitud','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10', 'value'=>$nroSolicitud, 'readonly'=>true];
            ...
            $columns = [];
            $columns[] = ['label'=> 'Artículos solicitados',
                'name'=>'articuloZafiro_id',
                'type'=>'datamodal',
                'datamodal_table'=>'articulosZafiro',
                'validation'=>'required','datamodal_columns_alias'=>'Monodroga, Descripción del artículo, Presentación, ID ARTÍCULO',
                'datamodal_columns'=>'des_monodroga,des_articulo,presentacion,id_articulo',
                'datamodal_size'=>'large',
                'datamodal_where' => 'id_familia IN (01, 02, 03, 04, 08, 09, 11, 13, 14, 99)',
                'datamodal_select_to'=>'presentacion_completa:presentacion',
                'required'=>true];
            ...
            $this->form[] = ['label'=>'Detalles de la solicitud', 'name'=>'pedidos_o2_detail', 'type'=>'child','table'=>'pedidos_o2_detail', 'foreign_key'=>'pedidos_o2_id', 'columns'=>$columns, 'width'=>'col-sm-10','required'=>true];


            $this->form[] = ['label'=>'Observaciones','name'=>'observaciones','type'=>'textarea','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];

            $this->form[] = ['label'=>'Archivo 1', 'name'=>'archivo1','type'=>'upload', 'help'=>'Archivos soportados PDF JPEG DOCX'];
			...
			//$this->form[] = ["label"=>"Archivo1","name"=>"archivo1","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			...
			//$this->form[] = ["label"=>"Nro Solicitud","name"=>"nro_solicitud","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
