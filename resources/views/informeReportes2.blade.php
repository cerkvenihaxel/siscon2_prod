...
<body>
    <div class="container">
        <h1 class="logo"> SISCON REPORTES - Sección Artículos</h1>

        <div class="card mb-24">
            <h5 class="card-header">Módulo de reportes para artículos</h5>
            <div class="card-body">
                <h5 class="card-title"></h5>
                <p class="card-text">Seleccione el número de artículo (puede consultar en panel de informes), luego el
                    período para el reporte. Presione buscar.</p>
    ...
    //$id < 9 ? $id=$dni : $id=$id; 
        ...
        $entrantes_detail=DB::table('entrantes_detail')->where('articulos_id', $id)->whereBetween('created_at', [$start_date, $end_date])->get();
            ...
            echo "<div class='table-responsive'>";
                echo "<table class='table table-hover'>";
                    ...
                    echo "<tbody>";
                        ...
                        echo "<td>".$d->afiliados_id = DB::table('afiliados')->where('id',
                            $d->afiliados_id)->value('apeynombres')."</td>";
                        echo "<td>".$medicos_id = DB::table('medicos')->where('id',
                            $d->medicos_id)->value('nombremedico')."</td>";
                        echo "<td>".$d->afiliados_id."</td>";
                        echo "<td>".$d->grupo_articulos = DB::table('grupos')->where('id',
                            $d->grupo_articulos)->value('des_grupo')."</td>";
                        echo "<td>".$d->cantidad = DB::table('entrantes_detail')->where('id',
                            $end->id)->where('articulos_id', $id)->value('cantidad')."</td>";
                        echo "</tr>";
                        }}
                        echo "</tbody>";
                ...
                echo "<table class='table table-hover'>";
                    ...
                    echo "<tbody>";
                        ...
                        echo "<tr key=".$cot->id.">";
                            ...
                            ";
                            echo "<td>".$unitario = DB::table('cotizaciones_detail')->where('entrantes_id',
                                $c->id)->value('precio_unitario')."</td>";
                            echo "<td>".$qty = DB::table('cotizaciones_detail')->where('entrantes_id',
                                $c->id)->value('cantidad')."</td>";
                            echo "<td>".$c->total."</td>";
                            echo "</tr>";
                        }}
                        echo "</tbody>";
